...
class Button(object):
	def __init__(self, colors, dim, text):
		...
		if self.textWidth > (self.width - 2): #2 = 1 px per side
			print(self.text[0] + ' buttons text is too wide :(')
		self.firstRender = 0
...
def rectCollision(rect1, rect2):
	...
	if rect2[0][0] <= rect1[0][0] <= rect2[0][1] or rect2[0][0] <= rect1[0][1] <= rect2[0][1] or rect1[0][0] <= rect2[0][0] <= rect1[0][1] or rect1[0][0] <= rect2[0][1] <= rect1[0][1]: #x1
		...
		return False
def newRectCollision(rect1, rect2):
	# Rect elements should be (x, y, width, height)
	#Should return False if there's no collision
	if rect1[0] <= rect2[0] <= rect1[0] + rect1[2] or rect1[0] <= rect2[0] + rect2[2] <= rect1[0] + rect1[2]: #Checking the collision at x cordinates
		print("X colliding")
		if rect1[1] <= rect2[1] <= rect1[1] + rect1[3] or rect1[1] <= rect2[1] + rect2[3] <= rect1[1] + rect1[3]: #Colliding with y TOO
			return True 
			print("collidng")
def newRectMesh(cord, width, height):
	x = cord[0]
	y = cord[1]
...
def shrinkAnimation():
	...
	pass
def loadDesign():
	if checkFileExisting('resources/design.txt'):
		dict = {}
		content = open('resources/design.txt', 'r').read()
		keyMarker = 0
...
def mainLoop():
	...
	testRectMesh = newRectMesh([300, 800], 50, 60)
	testRectColor = (255, 255, 255)
	testRectHoverColor = (155, 155, 155)
	...
	while  not loopExit:
		...
		noRender = False
		mouseMesh = newRectMesh(pygame.mouse.get_pos(), 1, 1)
		...
		if activeLoops['gameIntro']: #Game intro loop
			...
			if startButton.updateButton([dim['startButton_pos_x'], dim['startButton_pos_y']], True, gameIntroButtonsActive):
				...
				noRender = True
			print(testRectMesh)
			if newRectCollision(testRectMesh, mouseMesh):
				pygame.draw.rect(gameDisplay, testRectHoverColor, testRectMesh)
			else:
				pygame.draw.rect(gameDisplay, testRectColor, testRectMesh)
