	...
	while  not loopExit:
		...
		if activeLoops['game'] and noRender == False: #Game loop
			if not playingGame == True:
				...
				if pausedAt:
					pausedSince = frameTime - pausedAt
					if pausedSince <= 3:
						paused = True
						pauseCountdown = 3 - pausedSince
						timeMultipiler = (pausedSince/3) * timeMultipiler
					else:
						pausedAt = False
						paused = False
				if keys[pygame.K_ESCAPE] and escPressed == False:  
					noRender = True
				...
				textToScreen("SCORE:" + str(score), colors.white, int(displayHeight/60), 2, 2, False, standardFont)
				...
				if paused:
					overlay = pygame.Surface((displayWidth, displayHeight), pygame.SRCALPHA)
					overlay.fill((0,0,0,180))
					gameDisplay.blit(overlay, (0,0))
					textToScreen(str(math.ceil(pauseCountdown)), colors.white, int(displayHeight/10), int(displayWidth/2), displayHeight/2, True, standardFont)
