...
def ballMovment(speed, direction, prevCordinate, multiplier):
	cordinates = []
	if direction[0] == 'LEFT':
		cordinates.append(prevCordinate[0] - speed[0] * multiplier)

	elif direction[0] == 'RIGHT':
		cordinates.append(prevCordinate[0] + speed[0] * multiplier)
	if direction[1] == 'UP':
		cordinates.append(prevCordinate[1] - speed[1] * multiplier)
...
def randomGrey(min, max):
	...
	return[g,g,g]
def defendBall(paddleHeight, dim):
	...
	movment.append(random.randint(dim['defendball_max_y_speed'], dim['defendball_max_y_speed']))
...
def createGameData():
	...
	'playingGame': True,
	'gameOver':   False}
	data['balls'] = [defendBall(data['paddleHeight'], dim)]
	return data
def mainLoop():
	print(colors.red)
	global dim
	...
	while  not loopExit:
		...
		if activeLoops['gameIntro']: #Game intro loop
			textToScreen("PONG", colors.white, dim['title_size_height'], dim['title_pos_x'], dim['title_pos_y'], (True, True), standardFont)
		...
		if activeLoops['game'] and noRender == False: #Game loop
			if not GD['playingGame'] == True:
				if GD['gameOver'] == True:
					textToScreen("GAME OVER", colors.white, dim['gameover_size_height'], dim['gameover_pos_x'], dim['gameover_pos_y'], (True, True), standardFont)
				...
				for i, ball in enumerate(GD['balls']):
					...
					if rectCollision(GD['paddleMesh'], ball['mesh']): #Ball coliding with paddle?
						...
						if ball['hitted'] == False:
							GD['balls'].append(defendBall(GD['paddleHeight'], dim))
						ball['hitted'] = True
		...
		if activeLoops['pause'] and noRender == False:
			if keys[pygame.K_ESCAPE] and GD['escPressed'] == False or continueButton.updateButton([dim['continueButton_pos_x'], dim['continueButton_pos_y']], True): # Om inte escape var nedtryckt sisst men är nedtryckt nu
				...
				GD['escPressed'] = False
			textToScreen("PAUSED", colors.white, dim['paused_size_height'], dim['paused_pos_x'], dim['paused_pos_y'], (True, True), standardFont)
