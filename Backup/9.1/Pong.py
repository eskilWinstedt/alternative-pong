...
def mainLoop():
	...
	activeLoops = {'gameIntro': True, 'pause': False, 'game': False, 'FPSMeter': False, 'modesScreen': False, 'pause': False, 'reallyExitPopup': False, 'gameOver': False, 'scoreboard': False}
	...
	while  not loopExit:
		...
		if activeLoops['modesScreen'] and noRender == False:
			...
			if defendButton.updateButton([dim['defendButton_pos_x'], dim['defendButton_pos_y']], True):
				global GD
				GD = createGameData()
				saveScore(99999 ,'\\defendScore.txt') #FORTSÄTT HÄR
		...
		if activeLoops['scoreboard']:
			...
			textToScreen('HIGHSCORE: 397', colors.white, dim['scoreboard_highscore_height'], dim['scoreboard_highscore_pos_x'], dim['scoreboard_highscore_pos_y'], (True, True), standardFont)
