...
design = { #Using i and ek instead of y and x because of a known bug
	...
	'appli_button_x': 6849,
	'appli_button_y': 7500,
	'resolution_selector_pos_x': 5000,
	'resolution_selector_pos_y': 2500,
	'resolution_selector_text_height': 205,
	'resolution_selector_name_pos_x': 2500,
	'resolution_selector_name_pos_y': 2500,
	'resolution_selector_name_text_height': 2500,
	'settings_back_button_pos_x': 2500,
	'settings_back_button_pos_y': 7500,
	'settings_auto_button_pos_x': 4675,
	'settings_auto_button_pos_y': 7500,
	'fullscreen_teckst_pos_x': 2500,
	'fullscreen_teckst_pos_y': 3300,
...
def reverseList(list):
	...
	return reversedList
def reverseIndex(index, highestIndex):
...
class CreateSelector(object): #A text with two buttons on either side. < text > The text is the value that you select
	'''Have to be rewritten so there's no code written x2. It should be more easy to use'''
	def __init__(self, values, fontSizeKey, font, buttonsColors, centred, boxSize = 1.8):
		...
		for i ,value in enumerate(values):
			...
			if lenght > longestValueLenght:
				...
				self.longestValue = value
		temp, textRect = textObject(self.longestValue, colors.white, self.fontSize, self.font) #I don't need temp(orary)
		self.textDim = [textRect[2], textRect[3]] #Text Dimensions. Used to define the size of the selector
		self.boxWidth = self.textDim[0] * self.boxSize #How much wider than the text should the box be?
		self.boxHeight = int(self.textDim[1] * self.boxSize) #How much higher than the text should the box be?
	...
	def updatePositionKeys(self, posKeys):
		self.posKeys = posKeys
	def updatePosition(self):
		'''Run after change of posKeys'''
		self.pos = [dim[self.posKeys[0]], dim[self.posKeys[1]]]
		if self.centred[0]:
			self.pos[0] -= self.boxWidth/2
		if self.centred[1]:
			self.pos[1] -= self.boxHeight/2
		self.previousButtonPos = self.pos
		self.boxPos = [self.pos[0] + self.buttonsSide, self.pos[1]]
		self.nextButtonPos = [self.boxPos[0] + self.boxWidth, self.pos[1]]
		self.textPos = [self.pos[0] + self.buttonsSide + int(self.boxWidth/2), self.pos[1] + int(self.buttonsSide/2)]
	def correctInternalParts(self):
		'''Corrects the position of all parts in the selector after res change'''
		#The position has changed and therefore everything else in the selector also has to change
		self.pos = [dim[self.posKeys[0]], dim[self.posKeys[1]]]
		if self.centred[0]:
			self.pos[0] -= self.boxWidth/2
		if self.centred[1]:
		...
		self.nextButtonPos = [self.boxPos[0] + self.boxWidth, self.pos[1]]
		self.textPos = [self.pos[0] + self.buttonsSide + int(self.boxWidth/2), self.pos[1] + int(self.buttonsSide/2)]
		self.previousButton = Button(self.buttonsColors, [self.buttonsSide, self.buttonsSide], ('<', self.fontSize, self.font)) #Overwriting the old button
		self.nextButton = Button(self.buttonsColors, [self.buttonsSide, self.buttonsSide], ('>', self.fontSize, self.font))     #     -- || --
	def update(self):
...
class CreateSelector2(object): #A text with two buttons on either side. < text > The text is the value that you select
	...
	def updateDims(self):
		...
		self.height = dim[self.heightKey]		#Calculates the height depending on the current screen resoultion.
		self.fontSize = dim[self.fontSizeKey]	#Calculates the font size depending on the current screen resoultion.
	...
	def fix(self):
		'''Corrects the position of all parts in the selector after res change'''
		self.updatePosition(self.pos)
		self.previousButton = Button(self.buttonsColors, [self.buttonsSide, self.buttonsSide], ('<', self.fontSize, self.font)) #Overwriting the old buttons
		self.nextButton = Button(self.buttonsColors, [self.buttonsSide, self.buttonsSide], ('>', self.fontSize, self.font))     #     -- || --
		#-------------IMPORTANT-------------
		#Currently the updateAllButtonDims() 
		#function updates all buttons with 
		#the default button dimensions.
		#Therefore it's easier (but bad and ugly)
		#to just redo the whole button for 
		#the moment.
		#----------------FIX----------------
		#Rewrite the updateAllButtonDims()
		#function to update with spesific 
		#dimKeys stored in every button class
	def update(self):
...
def updateAllSelectorDims():
	for selector in allSelectors:
		selector.fontSize = dim[selector.fontSizeKey]
		temp, textRect = textObject(selector.longestValue, colors.white, selector.fontSize, selector.font)
		selector.textDim = [textRect[2], textRect[3]] #Text Dimensions. Used to define the size of the selector
		selector.boxWidth = selector.textDim[0] * selector.boxSize #How much wider than the text should the box be?
		selector.boxHeight = int(selector.textDim[1] * selector.boxSize) #How much higher than the text should the box be?
		selector.buttonsSide = selector.boxHeight #The buttons sidelenght. This variable is not necessary but makes the code easier to understand
		selector.correctInternalParts()
...
def mainLoop():
	...
	starOverlay, starOverlayRect = createOverlay(150)
	buttonsActive = True
	buttonsActive = True
	buttonsActive = True
	loopExit = False
	...
	ResolutionChanger = CreateSelector(resolutionsToValues(avilableResolutions), 'resolution_selector_text_height', standardFont, selectorButtonColorScheme, (False, False))
	ResolutionChanger.updatePositionKeys(['resolution_selector_pos_x', 'resolution_selector_pos_y'])
	ResolutionChanger.updatePosition()
	ResolutionChanger.selectedValue = len(ResolutionChanger.values) - 1 #Highest value (resolution)
	global frameTime
	print(str(dim['button_height']) + 'Knapp')
	#Start of the loops:
	while  not loopExit:
		...
		if activeLoops['settings']:
			...
			if settingsBackButton.updateButton([dim['settings_back_button_pos_x'], dim['settings_back_button_pos_y']], False):
				...
				noRender = True
			if settingsAutoButton.updateButton([dim['settings_auto_button_pos_x'], dim['settings_auto_button_pos_y']], False):
				#ResolutionChanger. FORTSÄTT HÄR
				print('This button is not functionall yet')
