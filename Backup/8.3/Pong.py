	...
	while  not loopExit:
		...
		if activeLoops['game'] and noRender == False: #Game loop
			...
			for i, ball in enumerate(GD['balls']):
				...
				if simpleRectCollision(GD['paddleMesh'], ball['mesh']): #Ball coliding with paddle?
					GD['score'] += 1
					if ball['hitted'] == False:
						...
						ball['hitted'] = True
				if simpleRectCollision(GD['lowerWall'], ball['mesh']):
					ball['movment'][1] = numberInvert(ball['movment'][1]) #Changing to the opposite direction by converting to negative cordinates
				if ball['lost'] == False and simpleRectCollision(GD['lostBallBorder'], ball['mesh']): #Check if ball is lost
					ball['lost'] = True
					GD['lives'] -= 1
					if GD['lives'] <= 0:
						activeLoops['gameOver'] = True
						activeLoops['game'] = False
						activeLoops['pause'] = False #There was a bug where the pause screen was visible when game over = was True, this is the fix
						noRender = True
					else:
						GD['balls'].append(defendBall())
				if simpleRectCollision(GD['upperWall'], ball['mesh']):
