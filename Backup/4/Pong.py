...
def createBall(x, y, direction, radius, movment, color = [0,0,0]):
	return {'cords': [x, y], 'direction': direction, 'hitted': False, 'lost': False, 'radius': radius, 'movment': movment, 'mesh': False, 'color': color}
...
def mainLoop():
	...
	while  not loopExit:
		...
		if activeLoops['game'] and noRender == False: #Game loop
			if not gameData['playingGame'] == True:
				if gameOver == True:
					...
					if gameData['restartButton'].updateButton([int(displayWidth/2), int(displayHeight/2)], True):
			else:
				...
				for i in gameData['balls']:
					gameData['ball'] = gameData['balls'].index(i) #ball is the index of the current ball in balls
					gameData['balls'][gameData['ball']]['cords'] = ballMovment(gameData['balls'][gameData['ball']]['movment'], gameData['balls'][gameData['ball']]['direction'], gameData['balls'][gameData['ball']]['cords'], timeMultipiler)
					gameData['balls'][gameData['ball']]['mesh'] = ballMesh(gameData['balls'][gameData['ball']]['cords'], gameData['balls'][gameData['ball']]['radius'])
					...
					if rectCollision(gameData['paddleMesh'], gameData['balls'][gameData['ball']]['mesh']):
						gameData['balls'][gameData['ball']]['direction'][0] = 'RIGHT'
						gameData['score'] += 1
						if gameData['balls'][gameData['ball']]['hitted'] == False:
							gameData['balls'].append(waveBall(paddleHeight))
						gameData['balls'[gameData['ball']]['hitted'] = True
					if rectCollision(lowerWall, balls[ball]['mesh']):        # FORTSÄTT HÄR FÖR HELVETE, FUCKTARD :|
						balls[ball]['direction'][1] = 'UP'
					if balls[ball]['lost'] == False and rectCollision(lostBallBorder, balls[ball]['mesh']):
						lives -= 1
						balls[ball]['lost'] = True
						if lives <= 0:
							print('It\'s over')
							gameOver = True
							playingGame = False
		...
		if activeLoops['pause'] and noRender == False:
			if keys[pygame.K_ESCAPE] and escPressed == False or continueButton.updateButton([int(displayWidth/2), int(displayHeight/2)], True):      # Om inte escape var nedtryckt sisst men är nedtryckt 
				noRender = True
