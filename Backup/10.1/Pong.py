...
def updateDesign():
	...
	return dict
def resolutionChanger(pos, availableResolutions, selectedResolution, data):
	previousButtonActive = True
	nextButtonActive = True
	text = str(availableResolutions[selectedResolution][0]) + 'x' + str(availableResolutions[selectedResolution][1])
	if data == None:
		data = {}
		data['numberAvailableResolutions'] = len(availableResolutions)
		print(availableResolutions)
		text = str(availableResolutions[selectedResolution][0]) + 'x' + str(availableResolutions[selectedResolution][1])
		highestResolutionText = str(availableResolutions[0][0]) + 'x' + str(availableResolutions[0][1])
		temp, textRect = textObject(highestResolutionText, colors.grey, dim['temp_height'], standardFont)
		textDim = [textRect[2], textRect[3]]
		data['boxWidth'] = textDim[0] * 1.2 #How much wider should the box be?
		data['boxHeight'] = textDim[1] * 1.2 #How much taller should the box be?		
		previousButton.width = data['boxHeight']
		previousButton.height = data['boxHeight']
		nextButton.width = data['boxHeight']
		nextButton.height = data['boxHeight']
		previousButton.colors[3] = colors.d_Grey
		nextButton.colors[3] = colors.d_Grey
		data['boxPos'] = [pos[0] + data['boxHeight'], pos[1]]
		data['button2Pos'] = [data['boxPos'][0] + data['boxWidth'], pos[1]]
		data['textPos'] = [data['boxPos'][0] + int(data['boxWidth']/2), pos[1] + int(data['boxHeight']/2)]
	if selectedResolution < data['numberAvailableResolutions'] - 1:
		previousButtonActive = True
	elif selectedResolution == data['numberAvailableResolutions'] - 1:
		previousButtonActive = False
	if selectedResolution > 0:
		nextButtonActive = True
	elif selectedResolution == 0:
		nextButtonActive = False
	if previousButton.updateButton(pos, False, previousButtonActive):
		selectedResolution += 1 #selecting previous resolution
	if nextButton.updateButton(data['button2Pos'], False, nextButtonActive):
		selectedResolution -= 1 #selecting next resolution
	pygame.draw.rect(gameDisplay, colors.grey, (data['boxPos'][0], data['boxPos'][1], data['boxWidth'], data['boxHeight']))
	textToScreen(text, colors.white, dim['temp_height'], data['textPos'][0], data['textPos'][1], (True, True), standardFont)
	return selectedResolution, data
def livesToScreen():
...
def mainLoop():
	...
	global dim
	dim = updateDesign()
	avilableResolutions = pygame.display.list_modes()
	...
	previousButton = Button(buttonColorScheme, buttonDim, ('<', buttonFontSize, standardFont))
	global previousButton
	nextButton = Button(buttonColorScheme, buttonDim, ('>', buttonFontSize, standardFont))
	global nextButton
	global frameTime
	#Start of the loops:
	while  not loopExit:
		...
		if activeLoops['settings']:
			...
			#TEMP
			selectedResolution, resolutionChangerData = resolutionChanger([dim['center_x'], dim['center_y']], avilableResolutions, selectedResolution, resolutionChangerData)
			#pygame.font.SysFont(standardFont, dim['temp_height']).size('1920x1080')[0]
			#textToScreen('1920x1080', colors.white, dim['temp_height'], dim['center_x'], dim['center_y'], (True, True), standardFont)
		...
		if surePopupSelection == 'YES':
			...
			if surePopupAction == 'EXIT':
				...
				print('Warning: no action assigned to surePopup!')
