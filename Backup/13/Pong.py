...
class CreateSelector2(object): #A text with two buttons on either side. < text > The text is the value that you select
	...
	def fix(self):
		...
		#the standard button dimensions.
		#Therefore it's easier (but bad and ugly)
		#to just recreate the whole button for 
		#the moment.
		#----------------FIX----------------
		#Rewrite the updateAllButtonDims()
		#function to update with spesific 
		#dimKeys stored in the button class.
	def update(self):
...
def mainLoop():
	...
	#ResolutionChanger = CreateSelector(resolutionsToValues(avilableResolutions), 'resolution_selector_text_height', standardFont, selectorButtonColorScheme, (False, False))
	#(self, values, dimKeys, font, buttonsColors, centred)
	ResolutionChanger = CreateSelector2(resolutionsToValues(avilableResolutions), ['resolution_selector_width', 'resolution_selector_height', 'resolution_selector_teckst_height'], standardFont, selectorButtonColorScheme, (False, False))
